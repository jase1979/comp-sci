...
```python
# Generating truth tables programmatically
...
for a in [0, 1]:
    for b in [0, 1]:
        print(f"{a} | {b} | {truth_table_and(a, b)}")

# Combined expression: Q = (A AND B) OR (NOT C)
print("\nQ = (A AND B) OR (NOT C)")
print("A | B | C | Q")
print("-" * 17)
for a in [0, 1]:
    for b in [0, 1]:
        for c in [0, 1]:
            q = truth_table_or(truth_table_and(a, b), truth_table_not(c))
            print(f"{a} | {b} | {c} | {q}")
```

```vb
' Generating truth tables programmatically
Module TruthTableDemo
    Function LogicAnd(a As Integer, b As Integer) As Integer
        Return a And b
    End Function

    Function LogicOr(a As Integer, b As Integer) As Integer
        Return a Or b
    End Function

    Function LogicNot(a As Integer) As Integer
        Return If(a = 0, 1, 0)
    End Function

    Function LogicXor(a As Integer, b As Integer) As Integer
        Return a Xor b
    End Function
    ...
    Sub Main()
        ...
        Console.WriteLine(New String("-"c, 13))
        For a As Integer = 0 To 1
            For b As Integer = 0 To 1
                Console.WriteLine($"{a} | {b} | {LogicAnd(a, b)}")
            Next
        Next

        ' Combined expression: Q = (A AND B) OR (NOT C)
        Console.WriteLine()
        Console.WriteLine("Q = (A AND B) OR (NOT C)")
        Console.WriteLine("A | B | C | Q")
        Console.WriteLine(New String("-"c, 17))
        For a As Integer = 0 To 1
            For b As Integer = 0 To 1
                For c As Integer = 0 To 1
                    Dim q As Integer = LogicOr(LogicAnd(a, b), LogicNot(c))
                    Console.WriteLine($"{a} | {b} | {c} | {q}")
                Next
            Next
        Next
...
```python
# Verifying Boolean simplification with truth tables
# Example: Show that (A AND B) OR (A AND NOT B) = A

print("Verifying: (A AND B) OR (A AND NOT B) = A")
print(f"{'A':>3} {'B':>3} | {'A.B':>5} {'A.~B':>5} | {'Original':>10} {'Simplified':>12}")
print("-" * 50)
...
for a in [0, 1]:
    for b in [0, 1]:
        ...
        original = ab | a_notb
        simplified = a
        match = "OK" if original == simplified else "FAIL"
        print(f"{a:>3} {b:>3} | {ab:>5} {a_notb:>5} | {original:>10} {simplified:>12}  {match}")

# Verify De Morgan's: NOT(A OR B) = (NOT A) AND (NOT B)
print("\nVerifying De Morgan's: NOT(A OR B) = (NOT A) AND (NOT B)")
print(f"{'A':>3} {'B':>3} | {'NOT(A OR B)':>12} {'NOT A AND NOT B':>16}")
print("-" * 40)
for a in [0, 1]:
    for b in [0, 1]:
        left = 1 - (a | b)
        right = (1 - a) & (1 - b)
        print(f"{a:>3} {b:>3} | {left:>12} {right:>16}")
```

```vb
' Verifying Boolean simplification with truth tables
Module BooleanSimplifyDemo
    Sub Main()
        ' Verify: (A AND B) OR (A AND NOT B) = A
        Console.WriteLine("Verifying: (A AND B) OR (A AND NOT B) = A")
        Console.WriteLine($"{"A",3} {"B",3} | {"A.B",5} {"A.~B",5} | {"Original",10} {"Simplified",12}")
        Console.WriteLine(New String("-"c, 50))

        For a As Integer = 0 To 1
            For b As Integer = 0 To 1
                Dim ab As Integer = a And b
                Dim aNotB As Integer = a And (1 - b)
                Dim original As Integer = ab Or aNotB
                Dim simplified As Integer = a
                Dim match As String = If(original = simplified, "OK", "FAIL")
                Console.WriteLine($"{a,3} {b,3} | {ab,5} {aNotB,5} | {original,10} {simplified,12}  {match}")
            Next
        Next

        ' Verify De Morgan's: NOT(A OR B) = (NOT A) AND (NOT B)
        Console.WriteLine()
        Console.WriteLine("Verifying De Morgan's: NOT(A OR B) = (NOT A) AND (NOT B)")
        Console.WriteLine($"{"A",3} {"B",3} | {"NOT(A OR B)",12} {"NOT A AND NOT B",16}")
        Console.WriteLine(New String("-"c, 40))
        For a As Integer = 0 To 1
            For b As Integer = 0 To 1
                Dim left As Integer = 1 - (a Or b)
                Dim right As Integer = (1 - a) And (1 - b)
                Console.WriteLine($"{a,3} {b,3} | {left,12} {right,16}")
            Next
        Next
    End Sub
End Module
